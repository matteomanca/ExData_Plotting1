...
plot( sub_df$Global_active_power , type="l",  
      ...
      axes=F, ann=T,      
      ylab="Global Active Power",xlab="", cex.lab=1, lwd=1)
...
text(c(0,sample_first_day,sample_first_day + sample_second_day), par("usr")[3] - .8,   #### set label on x axis; par("usr")[3] - 1 specifies the distance of labels from axis
     adj=1,
     labels=days,
...
##plot 2 - top right########################################
################################################
plot( sub_df$Voltage , type="l",  
      ylim=range(sub_df$Voltage),           
      axes=F, ann=T,      
      ylab="Voltage",xlab="datetime", cex.lab=1, lwd=1)
axis(1, lab=F ,at=c(0,sample_first_day,sample_first_day + sample_second_day))  ###### create x axis specifying the number and position of ticks    
...
##plot 4 - bottom right########################################
################################################
plot( sub_df$Global_reactive_power , type="l",  
      ylim=range(sub_df$Global_reactive_power),           
      axes=F, ann=T,      
      ylab="Global_reactive_power",xlab="datetime", cex.lab=1, lwd=1)
axis(1, lab=F ,at=c(0,sample_first_day,sample_first_day + sample_second_day))  ###### create x axis specifying the number and position of ticks    

text(c(0,sample_first_day,sample_first_day + sample_second_day), par("usr")[3]- 0.1 ,   #### set label on x axis; par("usr")[3] - 1 specifies the distance of labels from axis
     adj=1,
     labels=days,
     xpd=T, cex=1, offset = 1)
